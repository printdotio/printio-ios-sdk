## Select Photos screen

![enter image description here][1]

A. Customize the title on the photo sources screen. Default value is "Select Photos".

- setTitleForPhotoSourcesScreen

https://github.com/printdotio/printio-ios-sdk/blob/master/docs/code_customization.md#--settitleforphotosourcesscreen

B. Hide icon for Upload Instructions text in Photo Sources screen. Default value is NO.

- hideIconForUploadInstructions

https://github.com/printdotio/printio-ios-sdk/blob/master/docs/code_customization.md#--hideiconforuploadinstructions


C. Required step to configure which photo sources you would like to offer within your application.

- setAvailablePhotoSources
- PIO_SM_PHONE
- PIO_SM_INSTAGRAM
- PIO_SM_FACEBOOK
- PIO_SM_PICASA
- PIO_SM_FLICKR
- PIO_SM_DROPBOX
- PIO_SM_PHOTOBUCKET

https://github.com/printdotio/printio-ios-sdk/blob/master/docs/code_customization.md#--setavailablephotosources

---

[1]: https://lh4.googleusercontent.com/-Ew1RyKW276Q/VEo538oemaI/AAAAAAAAAP0/XZj6R4p4_pI/w501-h889-no/3.png
