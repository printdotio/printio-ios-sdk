...
#### A. Method to change background image for the Featured Products screen. Default appearance is white color.
...
#### A. Set text color for left label and right label on Items on Featured Products list.
...
#### A. Hide Category/Search view in Featured Products screen. Default value is NO.
...
#### A. Allow user to show/hide country on Products screen. Default value is YES. 
	Missing in new docs
...
#### A. Method to change background color on Select Country bar on Featured Products screen.
	Missing in new docs
